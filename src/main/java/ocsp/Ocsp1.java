...
public class Ocsp1 {
	...
	final static AlgorithmIdentifier HASH_SHA256 = new AlgorithmIdentifier(NISTObjectIdentifiers.id_sha256,
			DERNull.INSTANCE);
	...
	public static void main(String[] args) throws Exception {
		...
		// PEMParser pr=new PEMParser(new
		// FileReader("data/ocsp/my-startssl.crt"));
		...
		// BigInteger("32D2DC143F7192775D62755072D5BF7C", 16);
		// startssl neplatny
		// BigInteger serialNumber=new
		// BigInteger("4E63CAB15005249AC7B0DD96E31546D5", 16);
		...
		// CertificateID certId=new CertificateID(provider.get(HASH_SHA256),
		...
		if (resp.getStatus() == OCSPResp.SUCCESSFUL) {
			...
			for (SingleResp sr : br.getResponses()) {
				...
				System.out.println(sr.getCertStatus());// null good, jinak
														// RevokedStatus/UnknownStatus
				if (sr.getCertStatus() != null) {
					...
					if (sr.getCertStatus() instanceof RevokedStatus) {
						...
						// aACompromise (10)
						if (((RevokedStatus) sr.getCertStatus()).hasRevocationReason())
							System.out.println(((RevokedStatus) sr.getCertStatus()).getRevocationReason());
